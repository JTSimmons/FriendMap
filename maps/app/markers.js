'use strict'; (function() { var markerIcon = L.Icon.extend({options: {
...
FriendMap.markers = {
	...
	getMarker: function(itemType, item) {
		...
		if (FriendMap.config.markerType === 1) {
			if (itemType === 'guardtower') { return FriendMap.markers.style1.guardtower; }
			if (itemType === 'player') { return FriendMap.markers.style1.player; }
			if (itemType === 'town') { return FriendMap.markers.style2.portal; }
			if (item.permanent) { return FriendMap.markers.style1.main; }
			return FriendMap.markers.style1['letter_' + item.name.charAt(0).toLowerCase()];
		}
		// Updated village size markers based on citizen count
		if (FriendMap.config.markerType === 2) {
			if (itemType === 'guardtower') { return FriendMap.markers.style2.guardtower; }
			if (itemType === 'player') { return FriendMap.markers.style2.player; }
			if (itemType === 'town') { return FriendMap.markers.style2.village_solo; }
            if (itemType === 'point') { return FriendMap.markers.style1.guardtower; }
			if (item.citizens < 2) { return FriendMap.markers.style2.village_solo; }
			if (item.citizens < 9) { return FriendMap.markers.style2.village_small; }
			return FriendMap.markers.style2.village_large;
		}
		// Text labels
		if (FriendMap.config.markerType === 3) {
			if (itemType === 'guardtower') { return FriendMap.markers.style3.guardtower; }
			if (itemType === 'player') { return FriendMap.markers.style3.player; }
			if (itemType === 'town') { return FriendMap.markers.style2.village_solo; }
			if (itemType === 'point') { return FriendMap.markers.style1.guardtower; }
			return L.divIcon({
				className: 'icon-textlabel',
				html: '<div class="icon-inner">' + FriendMap.util.escapeHtml(item.name) + '</div>'
			});
		}
		throw new Error('Invalid marker type config value: ' + FriendMap.config.markerType);
	},
